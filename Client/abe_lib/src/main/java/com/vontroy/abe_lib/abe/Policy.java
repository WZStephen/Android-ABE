...
import com.vontroy.abe_lib.utils.Parser;
import com.vontroy.abe_lib.utils.PolicySyntaxException;
...
public class Policy {
    ...
    private void compute() throws PolicySyntaxException {
        ...
        BinaryTree.TreeNode root = new Parser().parse(policy);
        ...
        LinkedList<BinaryTree.TreeNode> queue = new LinkedList<BinaryTree.TreeNode>();
        queue.add(root);

        while (!queue.isEmpty()) {
            BinaryTree.TreeNode p = queue.removeFirst();
            if (p.getType() == BinaryTree.NodeType.AND) {
                ...
                BinaryTree.TreeNode right = p.getRight();
                ...
                BinaryTree.TreeNode left = p.getLeft();
                ...
                c += 1;
            } else if (p.getType() == BinaryTree.NodeType.OR) {
                BinaryTree.TreeNode left = p.getLeft();
                LinkedList<Integer> lv = new LinkedList<Integer>();
